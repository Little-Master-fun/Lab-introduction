...
<template>
  ...
  >
    ...
    <div class="p-6">
      ...
      >
        <template v-for="(link, linkIndex) in person.links" :key="linkIndex">
          <!-- 有URL时显示为链接 -->
          <a
            v-if="link.url"
            :href="link.url"
          ...
          <span
            v-else
            @click.stop
            class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-full cursor-default"
            :class="{
              'bg-blue-100 text-blue-700': link.type === 'cv',
              'bg-green-100 text-green-700': link.type === 'scholar',
              'bg-gray-100 text-gray-700': link.type === 'email',
            }"
          >
            <!-- CV Icon -->
            <svg
